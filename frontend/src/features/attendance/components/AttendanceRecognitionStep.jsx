...
export default function RecognitionStep({ 
  ...
  // Biometric Sync (Optimized to run once carefully)
  useEffect(() => {
    if (!globalFaceApi || !profiles || isDataSynced) return;
    ...
    try {
      const descriptors = profileList
        .filter(p => p.embedding && (Array.isArray(p.embedding) || p.embedding instanceof Float32Array))
      ...
      if (descriptors.length > 0) {
        // Cache the matcher to avoid per-frame overhead
        faceMatcherRef.current = new faceapi.FaceMatcher(descriptors, RECOGNITION_THRESHOLD);
        setIsDataSynced(true);
        console.log(`[Biometric] Synced ${descriptors.length} identities.`);
      }
    } catch (err) {
      console.error("[Biometric] Sync Error:", err);
    }
  }, [profiles, globalFaceApi, isDataSynced]);
  ...
  const handleTouchMove = (e) => {
    if (e.touches.length === 2 && prevPinchDistRef.current && capabilities?.zoom) {
  ...
  const runTrackingLoop = useCallback(() => {
    const detectAndMatch = async () => {
      if (!videoRef.current || videoRef.current.paused || !faceApiRef.current) return;
      ...
      try {
        ...
        if (canvasRef.current.width !== vw) faceapi.matchDimensions(canvasRef.current, displaySize);
          ...
          if (faceMatcherRef.current) {
            resizedDetections.forEach(det => {
              ...
              if (bestMatch.label !== "unknown") {
                ...
                if (profile) {
                  currentMatches.push({ id: profile.userId, name: profile.name, avatar: profile.avatar, box: det.detection.box });
                  if (!markedInLoopRef.current.has(profile.userId)) {
                    onMarkPresent(profile.userId, "face");
                    markedInLoopRef.current.add(profile.userId);
                    setTimeout(() => markedInLoopRef.current.delete(profile.userId), 5000);
                  }
                }
              }
            });
      ...
      animFrameRef.current = requestAnimationFrame(() => setTimeout(detectAndMatch, 150));
    };
    detectAndMatch();
  ...
  return (
    <div ref={containerRef} className="relative h-full w-full overflow-hidden flex flex-col bg-black lg:rounded-3xl">
      <div 
        className="relative flex-1 touch-none overflow-hidden"
        onTouchStart={(e) => e.touches.length === 2 && (prevPinchDistRef.current = Math.hypot(e.touches[0].pageX - e.touches[1].pageX, e.touches[0].pageY - e.touches[1].pageY))}
        onTouchMove={handleTouchMove}
        onTouchEnd={() => (prevPinchDistRef.current = null)}
      >
        <video ref={videoRef} muted playsInline className="absolute inset-0 w-full h-full object-cover" />
        <canvas ref={canvasRef} className="absolute inset-0 w-full h-full object-cover pointer-events-none z-10 opacity-60" />

        <div className="absolute inset-0 pointer-events-none z-20">
          {activeMatches.map((match) => (
            <div key={match.id} className="absolute transition-all duration-200" style={{
              ...
              <div className="relative -top-14 left-1/2 -translate-x-1/2 flex flex-col items-center animate-in zoom-in-75 duration-300">
                <div className="bg-background/80 backdrop-blur-xl border-2 border-primary rounded-2xl p-1.5 shadow-2xl flex items-center gap-2 min-w-[120px]">
                  <Avatar className="size-7 border-2 border-background shadow-sm shrink-0">
                    ...
                    <AvatarFallback className="text-[10px] bg-primary/20 text-primary font-black">{match.name.charAt(0)}</AvatarFallback>
                  </Avatar>
                  <div className="flex flex-col min-w-0 pr-1">
                    <span className="text-[10px] font-black text-foreground truncate uppercase leading-none mb-1">{match.name}</span>
                    <div className="flex items-center gap-1">
                      <div className="size-1 bg-emerald-500 rounded-full" />
                      <span className="text-[8px] font-black text-emerald-600 uppercase">Recognized</span>
                    </div>
                  </div>
                </div>
        ...
        <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-6 z-30">
          <div className="flex justify-between items-start pointer-events-auto">
            <div className="bg-black/40 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3">
               ...
               <span className="text-[9px] font-black uppercase tracking-widest text-white/90">Biometric Live</span>
            </div>
            <div className="flex gap-3">
              <Button variant="secondary" size="icon" className="size-11 rounded-2xl bg-black/40 border border-white/10 text-white" onClick={() => setFacingMode(f => f === "user" ? "environment" : "user")}>
                <RefreshCw className="size-5" />
              </Button>
              <Button variant="secondary" size="icon" className="size-11 rounded-2xl bg-black/40 border border-white/10 text-white" onClick={toggleFullscreen}>
            ...
            <div className="w-full flex items-center justify-between pointer-events-auto">
               <div className="flex flex-col">
                  <span className="text-3xl font-black text-white shadow-black drop-shadow-lg tabular-nums">
                    {markedCount}<span className="text-xs opacity-40 ml-1">/{students.length}</span>
                  </span>
                  <span className="text-[9px] font-black text-white/50 uppercase tracking-widest">Marked Present</span>
               </div>
               <Button onClick={onComplete} className="rounded-2xl h-14 px-8 font-black text-xs uppercase tracking-[0.2em] shadow-2xl bg-white text-black hover:bg-white/90 active:scale-95 transition-all">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black z-50 p-12">
             <div className="w-full max-w-[280px] space-y-8 animate-in fade-in duration-700">
                <div className="flex justify-center mb-4">
                  <div className="size-16 rounded-3xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center">
                    {!isDataSynced ? <Database className="size-8 text-emerald-500" /> : <Cpu className="size-8 text-emerald-500" />}
                  </div>
                </div>
                
                <div className="space-y-3">
                  <div className="flex justify-between text-[10px] font-black uppercase tracking-[0.2em] text-white/40">
                    <span>{ !isDataSynced ? "Syncing Biometrics" : "Calibrating Neural Net" }</span>
