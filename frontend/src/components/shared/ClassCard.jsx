...
const ClassCard = memo(({ 
  ...
  const duration = calculateDuration(cls.schedule?.startTime, cls.schedule?.endTime);
        ...
        <div className="flex items-start justify-between mb-4 gap-2">
          <div className="flex items-center gap-4 min-w-0">
            <div className={cn(
              ...
              variant === "highlight" ? "bg-emerald-500 text-white border-emerald-400" : "bg-accent text-accent-foreground",
            ...
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-2">
                ...
                {isArchived && (
                  <Badge variant="outline" className="rounded-full px-2 py-0 font-black text-[8px] uppercase tracking-wider text-muted-foreground border-muted-foreground/20 bg-muted/20 shrink-0">
                    Archived
                  </Badge>
                )}
              </div>
              <div className="flex items-center gap-1 text-[11px] font-bold text-muted-foreground mt-0.5">
                ...
                <span className="truncate">
                  {cls.schedule?.room ? formatRoom(cls.schedule.room) : "No Room Set"}
                </span>
              </div>
            </div>
          </div>
          
          <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()} aria-label="Class management actions">
            {badge}
            {actions}
          </div>
        </div>
        ...
        <div className="space-y-1.5 flex-1">
           <div className="flex items-center gap-2 text-[13px]">
             ...
             <div className="flex items-baseline gap-1.5">
               <span className="font-bold text-foreground/90">
                 {cls.schedule?.startTime ? format12Hour(cls.schedule.startTime) : "--"} - {cls.schedule?.endTime ? format12Hour(cls.schedule.endTime) : "--"}
               </span>
               {duration && (
           ...
           <div className="flex items-center gap-2 text-[13px]">
             ...
             <span className="font-bold text-foreground/90 truncate">
               {cls.schedule?.days?.length > 0 ? formatDays(cls.schedule.days) : "No days set"}
             </span>
           </div>
           ...
           <div className="flex items-center gap-2 text-[13px]">
             ...
             <span className="font-bold text-foreground/90">
               {cls.studentCount || cls.studentIds?.length || 0} Students Enrolled
             </span>
           </div>
        </div>
