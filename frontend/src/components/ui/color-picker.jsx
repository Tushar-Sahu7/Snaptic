...
// Parse oklch() string to Color object
function safeParseColor(val) {
  if (!val) return null;
  ...
  if (oklchMatch) {
    ...
    const r = clamp(gamma(lr));
    const g = clamp(gamma(lg));
    const bVal = clamp(gamma(lb));
    
    try {
      return Color.rgb(r, g, bVal).alpha(A);
    } catch {
      return Color('#000000');
    }
  }
...
export const ColorPicker = ({
  ...
  const selectedColor = safeParseColor(value) || Color(defaultValue)
